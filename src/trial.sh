...
mv INTCODE $1.i
./icint -l $1.int
mv icint.log $1.iclog
./icint xg0.int < $1.int
mv ASM $1.s
sed -e 's=a5=M=g' -e 's=s4=G=g' -e 's=s5=P=g' -e 's=s6=A=g' -e 's=s7=B=g' -e 's=s9=D=g' $1.s > $1.asm
gcc -g -o $1.exe -static blib.c $1.s
objdump -d $1.exe | sed -e 's=a5=M=g' -e 's=s4=G=g' -e 's=s5=P=g' -e 's=s6=A=g' -e 's=s7=B=g' -e 's=s9=D=g' > $1.dis
rm -f logfile.txt
gdb --batch-silent ./$1.exe -x commands.gdb 
